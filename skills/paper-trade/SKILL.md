...
The CLI is installed at `/root/.openclaw/skills/paper-trade/`. To use it:

```bash
/root/.openclaw/skills/paper-trade/.venv/bin/alpaca <command>
```

Config lives at `~/.alpaca-cli/config.json`. API keys are already configured.
